...
@section('content')
    <div class="max-w-7xl mx-auto space-y-6 pb-12" x-data="calendarApp()" x-init="initCalendar(); initDashboardStats()">
        ...
        @if (isset($maintenanceAlerts) && count($maintenanceAlerts) > 0)
            <div
                class="bg-red-50 rounded-2xl shadow-sm border border-red-200 overflow-hidden relative p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div
                        class="bg-red-100 w-12 h-12 rounded-full flex items-center justify-center text-red-600 animate-pulse shrink-0">
                        <i class="fa-solid fa-triangle-exclamation text-2xl"></i>
                    </div>
                    <div>
                        <h3 class="font-bold text-red-800 text-lg">แจ้งเตือนด่วน: เครื่องจักรถึงรอบซ่อมบำรุง
                            ({{ count($maintenanceAlerts) }} คัน)</h3>
                        <p class="text-sm text-red-600">กรุณาตรวจสอบและเปิดใบแจ้งซ่อม
                            เพื่อป้องกันความเสียหายระหว่างปฏิบัติงาน</p>
                    </div>
                </div>
                <a href="{{ route('admin.maintenance.index') }}"
                    class="shrink-0 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-xl shadow-md transition w-full sm:w-auto text-center">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            {{-- Pending Jobs (สำคัญที่สุด ต้องจัดการก่อน) --}}
                <a href="{{ route('admin.jobs.index', ['status' => 'pending_approval,completed_pending_approval']) }}"
                class="bg-orange-50 p-5 rounded-2xl shadow-sm border border-orange-100 flex flex-col justify-between h-32 relative overflow-hidden group hover:shadow-md transition-all hover:-translate-y-1">
                ...
                <div>
                    <p class="text-sm text-orange-800 font-bold flex items-center gap-1.5"><i
                            class="fa-solid fa-circle-exclamation animate-pulse text-orange-500"></i> รอตรวจสอบ/อนุมัติ</p>
                        <h3 class="text-3xl font-black text-orange-600 mt-1"><span x-text="stats.pending_jobs">{{ $dashboardStats['pending_jobs'] ?? 0 }}</span> <span
            ...
            {{-- Active Machines --}}
            <div
                class="bg-blue-50 p-5 rounded-2xl shadow-sm border border-blue-100 flex flex-col justify-between h-32 relative overflow-hidden group hover:shadow-md transition-all">
                <div class="absolute right-0 top-0 p-3 opacity-10 group-hover:scale-125 transition-transform duration-500">
                    <i class="fa-solid fa-tractor text-6xl text-blue-600"></i>
                </div>
                <div>
                    <p class="text-sm text-blue-800 font-bold flex items-center gap-1.5"><i
                            class="fa-solid fa-circle text-[8px] text-blue-500 animate-ping"></i> กำลังทำงานตอนนี้</p>
                        <h3 class="text-3xl font-black text-blue-600 mt-1"><span x-text="stats.active_machines">{{ $dashboardStats['active_machines'] ?? 0 }}</span> <span
            ...
            {{-- Available Staff --}}
            <div
                class="bg-purple-50 p-5 rounded-2xl shadow-sm border border-purple-100 flex flex-col justify-between h-32 relative overflow-hidden group hover:shadow-md transition-all">
                <div class="absolute right-0 top-0 p-3 opacity-10 group-hover:scale-125 transition-transform duration-500">
                    <i class="fa-solid fa-users text-6xl text-purple-600"></i>
                </div>
                <div>
                    <p class="text-sm text-purple-800 font-bold">พนักงานในระบบ</p>
                        <h3 class="text-3xl font-black text-purple-600 mt-1"><span x-text="stats.available_staff">{{ $availableStaff ?? 0 }}</span> <span
            ...
            <div
                class="bg-green-50 p-5 rounded-2xl shadow-sm border border-green-100 flex flex-col justify-between h-32 relative overflow-hidden group hover:shadow-md transition-all">
                <div class="absolute right-0 top-0 p-3 opacity-10 group-hover:scale-125 transition-transform duration-500">
                ...
                <div>
                    <p class="text-sm text-green-800 font-bold">งานเสร็จสิ้น (เดือนนี้)</p>
                        <h3 class="text-3xl font-black text-green-600 mt-1"><span x-text="stats.completed_jobs">{{ $completedJobs ?? 0 }}</span> <span
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-[400px]">
                <div class="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    ...
                    <span
                        class="text-xs bg-agri-primary text-white px-2 py-1 rounded-lg font-bold">{{ date('d M Y') }}</span>
                </div>
                <div class="p-4 flex-1 overflow-y-auto custom-scrollbar space-y-3">
                    @forelse ($todayJobs ?? [] as $job)
                        ...
                        @endphp
                        <div
                            class="border border-gray-100 p-3 rounded-xl hover:border-agri-primary/50 transition bg-white shadow-sm flex flex-col justify-between">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex items-center gap-2">
                                    <span class="font-bold text-gray-800">{{ $job->job_number }}</span>
                                    {{-- 🟢 ป้ายสถานะ --}}
                                    <span
                                        class="text-[10px] px-2 py-0.5 rounded-md border font-bold flex items-center {{ $statusConfig['color'] }}">
                                        <i class="fa-solid {{ $statusConfig['icon'] }} mr-1"></i>
                                        {{ $statusConfig['label'] }}
                                    </span>
                                </div>
                                <span
                                    class="text-[10px] bg-gray-50 text-gray-600 px-2 py-1 rounded-lg font-bold border border-gray-100 whitespace-nowrap">
                            ...
                            <div class="flex justify-between items-end">
                                <div>
                                    <p class="text-sm text-gray-600 mb-1"><i
                                            class="fa-solid fa-user text-gray-400 text-xs w-4"></i>
                                        {{ $job->customer_name }}</p>
                                    <p class="text-xs text-gray-500"><i
                                            class="fa-solid fa-tractor text-orange-400 text-xs w-4"></i>
                                        {{ $job->equipment_name }}</p>
                                </div>
                                {{-- 🟢 ปุ่มดูรายละเอียด --}}
                                @if (isset($job->id))
                                    <a href="{{ route('admin.jobs.show', $job->id) }}"
                                        class="text-[10px] bg-white border border-gray-200 text-gray-600 px-2 py-1 rounded-md hover:bg-gray-50 transition shadow-sm font-medium">
                ...
                <div class="p-4 flex-1 overflow-y-auto custom-scrollbar space-y-3">
                    ...
                    @foreach ($overdueJobs ?? [] as $job)
                        <div class="flex items-center justify-between border-l-4 border-red-500 bg-red-50 p-3 rounded-r-xl">
                            ...
                            <a href="{{ route('admin.jobs.show', $job->id) }}"
                                class="shrink-0 px-3 py-1.5 bg-red-600 text-white hover:bg-red-700 text-xs font-bold rounded-lg transition shadow-sm ml-2">
                                ติดตามงาน
                            </a>
                        </div>
                    @endforeach
                    ...
                    @foreach ($expiredPendingJobs ?? [] as $job)
                        <div
                            class="flex items-center justify-between border-l-4 border-gray-500 bg-gray-100 p-3 rounded-r-xl">
                            <div>
                            ...
                            <a href="{{ route('admin.jobs.show', $job->id) }}"
                                class="shrink-0 px-3 py-1.5 bg-white border border-gray-300 text-gray-700 hover:bg-gray-200 text-xs font-bold rounded-lg transition ml-2">
                                จัดการ
                            </a>
                        </div>
                    @endforeach
                    ...
                    @forelse ($pendingJobs ?? [] as $job)
                        <div
                            class="flex items-center justify-between border-l-4 border-orange-400 bg-orange-50/50 p-3 rounded-r-xl">
                            <div>
                                <p class="text-sm font-bold text-gray-800">{{ $job->job_number }}</p>
                                <p class="text-xs text-gray-500 mt-0.5">รออนุมัติ / นัด:
                                    {{ \Carbon\Carbon::parse($job->scheduled_start)->format('d/m/Y') }}</p>
                            </div>
                            <a href="{{ route('admin.jobs.show', $job->id) }}"
                                class="shrink-0 px-3 py-1.5 bg-white border border-orange-200 text-orange-600 hover:bg-orange-600 hover:text-white text-xs font-bold rounded-lg transition ml-2">
                                พิจารณา
                            </a>
                        </div>
                    @empty
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden relative">
            <div
                class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row items-center justify-between bg-gray-50 gap-4">
                ...
                <div class="flex items-center gap-3">
                    <div class="flex items-center gap-3 bg-white p-1 rounded-xl border border-gray-200 shadow-sm">
                        <button @click="prevMonth()"
                            class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-600 transition">
                    ...
                    <button @click="goToToday()"
                        class="px-4 py-2 rounded-xl bg-white border border-gray-200 text-xs font-bold text-gray-700 hover:bg-gray-50 transition">
                        วันนี้
                    </button>
                </div>
            </div>
                        ...
                        <div class="relative h-24 md:h-32 border border-gray-200 rounded-lg p-1 hover:border-agri-primary transition flex flex-col overflow-hidden"
                            ...
                            <div class="flex-1 overflow-y-auto space-y-1 custom-scrollbar">
                                <template x-for="event in getEvents(date)">
                                    <div class="w-full text-left px-1.5 py-1 rounded text-[10px] truncate border"
        ...
        function calendarApp() {
            return {
                ...
                initDashboardStats() {
                    ...
                    this.statsRefreshTimer = setInterval(() => this.refreshOperationalStats(), 15000);
                },

                async refreshOperationalStats() {
                    try {
                        const response = await fetch("{{ route('admin.dashboard.operational') }}", {
                            headers: {
                        ...
                        console.debug('refreshOperationalStats failed', error);
                    }
                },
